...
template <typename T> const node<T> *cross_bridge_right(const node<T> &n) {
  const node<T> *prev = &n;
...
template <typename T> const node<T> *cross_bridge_left(const node<T> &n) {
  const node<T> *prev = &n;
  const node<T> *next = n.parent();
  while (next) {
...
const node<T> *descent(const node<T> &n, Callable call) {
  static_assert(
      std::is_convertible<
          Callable, std::function<const node<T> *(const node<T> &)>>::value,
