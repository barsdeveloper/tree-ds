...
class tree_base {
     ...
     */
    constexpr tree_base() = default;

    explicit tree_base(const Allocator& allocator) :
            allocator(allocator) {
    }
     ...
     */
    explicit tree_base(const tree_base& other) :
    ...
    explicit tree_base(const tree_base<T, Node, OtherPolicy, OtherAllocator>& other) :
            root(
                other.root
                    ? allocate(allocator, *other.root, allocator).release()
                    : nullptr),
    ...
    tree_base(tree_base<T, Node, OtherPolicy, Allocator>&& other) :
            allocator(other.allocator),
            root(other.root),
            size_value(other.size_value) {
        other.emptify();
    ...
    tree_base(const struct_node<ConvertibleT, Children...>& root) :
    ...
    tree_base(const struct_node<std::tuple<EmplacingArgs...>, Children...>& root) :
            root(allocate(allocator, root, allocator).release()),
            size_value(root.get_subtree_size()) {
    }
    ...
    tree_base& operator=(const tree_base<T, Node, OtherPolicy, OtherAllocator>& other) {
        static_assert(
    ...
    template <typename OtherPolicy>
    tree_base& operator=(tree_base<T, Node, OtherPolicy, Allocator>&& other) {
        allocator = other.allocator;
    ...
    template <
        ...
        typename... Nodes,
        CHECK_CONVERTIBLE(ConvertibleT, T)>
    tree_base& operator=(const struct_node<ConvertibleT, Nodes...>& root) {
        this->assign(
    ...
    template <
        ...
        typename... Children,
        CHECK_CONSTRUCTIBLE(value_type, EmplacingArgs...)>
    tree_base& operator=(const struct_node<std::tuple<EmplacingArgs...>, Children...>& root) {
        this->assign(
            allocate(allocator, root, allocator).release(),
            root.get_subtree_size());
        return *this;
    ...
    tree_iterator<tree_base, P, Constant>
    modify_subtree(
        tree_iterator<tree_base, P, Constant> position,
    ...
    //  ---   COMPARISON   ---
    template <typename OtherPolicy, typename OtherAllocator>
    bool operator==(const tree_base<T, Node, OtherPolicy, OtherAllocator>& other) const {
...
template <
    ...
    typename Allocator1,
    typename Allocator2>
bool operator!=(const tree_base<T, Node, Policy1, Allocator1>& lhs, const tree_base<T, Node, Policy2, Allocator2>& rhs) {
