...
public:
	...
	tree_iterator& operator ++() {
		if (current) {
			current = const_cast<node_type*>(algorithm.increment(*current)); // const_cast needed in case node_type is non const
		} else if (tree && tree->_root) { // If iterator is at the end()
			...
			current = const_cast<node_type*>(algorithm.go_first(*tree->_root)); // const_cast needed in case node_type is non const
		}
		return *this;
	}
	...
	tree_iterator& operator --() {
		if (current) {
			...
			current = const_cast<node_type*>(algorithm.decrement(*current)); // const_cast needed in case node_type is non const
			if (!current) { // if decrementation caused past the begin() ...
				current = temp; // ... then leave current node as it was
			}
		} else if (tree && tree->_root) { // if iterator is at the end()
			...
			// REMEMBER: ++ operator on a reverse_iterator delegates to -- operator of tree_iterator and vice versa
			current = const_cast<node_type*>(algorithm.go_last(*tree->_root)); // const_cast needed in case node_type is non const
		}
		return *this;
	}
